...
public class ApiClient {
  ...
  /**
   * Configure whether to verify certificate and hostname when making https requests. Default to true. NOTE: Do NOT set to false in
   * production code, otherwise you would face multiple types of cryptographic attacks.
   *
  ...
  /**
   * The path of temporary folder used to store downloaded files from endpoints with file response. The default value is <code>null</code>,
  ...
  public int getConnectTimeout() {
    return httpClientBuilder.build().connectTimeoutMillis();
  }

  /**
   * Sets the connect timeout (in milliseconds). A value of 0 means no timeout, otherwise values must be between 1 and {@link
   * Integer#MAX_VALUE}.
   *
  ...
  public int getReadTimeout() {
    return httpClientBuilder.build().readTimeoutMillis();
  }

  /**
   * Sets the read timeout (in milliseconds). A value of 0 means no timeout, otherwise values must be between 1 and {@link
   * Integer#MAX_VALUE}.
   *
  ...
  public int getWriteTimeout() {
    return httpClientBuilder.build().writeTimeoutMillis();
  }

  /**
   * Sets the write timeout (in milliseconds). A value of 0 means no timeout, otherwise values must be between 1 and {@link
   * Integer#MAX_VALUE}.
   *
  ...
  /**
   * Select the Accept header's value from the given accepts array: if JSON exists in the given array, use it; otherwise use all of them
  ...
  /**
   * Select the Content-Type header's value from the given array: if JSON exists in the given array, use it; otherwise use the first one of
   * the array.
   *
   ...
   * @param call       Call
   * @return ApiResponse object containing response status, headers and data, which is a Java object deserialized from response body and
   * would be null when returnType is null.
   * @throws ApiException If fail to execute the call
   */
  ...
  public Call buildCall(String path, String method, List<Pair> queryParams, List<Pair> collectionQueryParams, Object body,
    ...
    return httpClientBuilder.build().newCall(request);
  }
